...
Functions:
    ...
    whittle_lik(data, freqs, response, sources, ...): Computes the Whittle likelihood.
    gamma_lik(data, freqs, response, sources, ...): Computes the Gamma likelihood.
...
def whittle_lik(data, freqs, response, sources, dt, t1, t2, dof, gen2):
    """
...
def gamma_lik(data, freqs, response, sources, dt, t1, t2, dof, gen2):
    """
...
def beta_scaled_log_likelihood(log_like_fn, beta):
    ...
    """
    def scaled_likelihood(data, freqs, response, sources, dt, t1, t2, dof, gen2):
        with numpyro.handlers.scale(scale=beta): 
            return log_like_fn(data, freqs, response, sources, dt, t1, t2, dof, gen2)
