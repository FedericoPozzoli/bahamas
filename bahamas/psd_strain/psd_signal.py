...
# -------------------- Dynamical friction sBBH SGWB model --------------------
...
def Omega_DF(freqs, par):
    """
    Dynamical friction model for gravitational wave signal with a bump (RPLP).
    See https://arxiv.org/pdf/2507.00694 for details.
    Args:
      freqs (array): Frequency array.
      par (dict): Dictionary containing:
      - log10A_vac        : log10 of the vacuum term coefficient
      - log10rho_normal   : log10 of normalized environmental density(e.g, 10^-10 -> 0, 10^-7 -> 3)
    Returns:
        array: Power spectral density.
    """
    log10A_vac = par['log10A_vac']
    A_vac = 10.0 ** log10A_vac
    A_m   = -2.72524488e-14
    log10rho_normal = par['log10rho_normal']

    f = jnp.maximum(jnp.asarray(freqs), 1e-12)  # numerical stability
    # Numerator
    numerator = A_vac * f**(2.0 / 3.0)

    # Denominator part 1: 1 + 10^rho * A_m * f^(-11/3) * ln(f)
    rho10 = 10.0**log10rho_normal
    denom_asym = 1.0 + rho10 * A_m * f**(-11.0 / 3.0) * jnp.log(f)
    denom_asym = jnp.clip(denom_asym, 1e-30, jnp.inf)  # avoid division by zero/negative

    # Gaussian correction term: (1 + Gauss)
    log_f_peak = -3.4059 + 0.2623 * log10rho_normal
    amplitude  =  1.2034 - 0.0262 * log10rho_normal
    sigma      =  0.2217
    gauss = amplitude * jnp.exp(-((jnp.log10(f) - log_f_peak) ** 2) / (2.0 * sigma**2))
    gauss_corr = 1.0 + gauss
    gauss_corr = jnp.clip(gauss_corr, 1e-30, jnp.inf)

    Omega = numerator / (denom_asym * gauss_corr)

    S_h = Omega * (3.0 * H02) / (4.0 * jnp.pi**2 * f**3)
    return S_h

# -------------------- Eccentric vacuum sBBH SGWB model --------------------
# Based on paper: https://arxiv.org/pdf/2605.05537
@jit
...
def _hc_squared_circular_jnp(fobs, z0, M0):
    prefactor = (4 * G * (jnp.pi * G)**(2/3)) / (3 * jnp.pi * c**2)
...
@jit
def Omega_ecc_vac(freqs, par):
    """
    Eccentric vacuum sBBH SGWB model.
    Args:
        freqs (array): Frequency array in Hz.
        par (dict): Dictionary containing:
            - log10A_vac : log10 of amplitude scaling factor
            - e0        : initial eccentricity (defined at fp0)
            - log10fp0   : (optional) log10 of orbital frequency fp0 in Hz 
                           (e.g., fp0 = 1e-4 corresponds to log10fp0 = -4)
    Returns:
        array: Power spectral density.
    """
    # Extract parameters
    log10A_vac = par['log10A_vac'] # -10.3036
    A_vac = 10.0 ** log10A_vac
    M_0 = 4.16e8 * msun
    z_0 = 0.02
    A_pop = A_vac / (8 * (jnp.pi * M_0 * G)**(5/3)/(9* H02 *(1+z_0)**(1/3)*c**2)*Mpc**(-3))
    e0 = par['e0']
    # log10fp0 = par['log10fp0']
    # fp0 = 10.0 ** log10fp0
    fp0 = 1e-4  # fixed fp0 for now

    # Avoid zero frequencies
    freqs = jnp.maximum(freqs, 1e-30)

    # Compute Omega_gw using the population model
    Omega = _omega_gwb_with_pop_jnp(freqs, A_pop, e0, fp0)
