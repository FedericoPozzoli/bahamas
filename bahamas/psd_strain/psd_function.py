...
def precompute_los_quadrature(PRECOMPUTED, D_max=20.0, N_quad=16, R_sun=8.12, z_sun=0.03):
    ...
    nodes, weights = _np.polynomial.legendre.leggauss(N_quad)  # numpy arrays
...
def model_psd(freqs, sources, response, injected=False, tdi=0, **kwargs):
    ...
    for source_name in sources.keys():
        ...
        if source_name == 'cosmic_string':
            ...
            psd += galactic_foreground_num(freqs, sources[source_name], t1=t1, t2=t2, tdi=tdi, injected=injected, gen2=kwargs.get('gen2'))
            if injected:
                true_psd.append(galactic_foreground_num(freqs, sources[source_name], t1=t1, t2=t2, tdi=tdi, injected=injected, gen2=kwargs.get('gen2')))
