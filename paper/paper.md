...
Similar to other works [@Rosati:2024], we use a Short-Time Fourier Transform likelihood rappresentation to analyze data in chunked segments. While the chunking procedure mitigates non-stationarity, it does not account for variations in spectral amplitude within chunks caused by modulation. [@Digman:2022] addresses this by introducing a phenomenological template that models the amplitude modulation as a superposition of sinusoidal forms.
